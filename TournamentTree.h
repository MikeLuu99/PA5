...
TournamentNode<T>* TournamentTree<T>::buildSingleEliminationTree(std::vector<T*>& competitors) {
    ...
    for (int i = 0; i < current.size(); i++) {
        TournamentNode<T>* node = new TournamentNode<T>(current[i]);
...
TournamentNode<T>* TournamentTree<T>::buildLosersBracket(std::vector<T*>& losers) {
    ...
    for (int i = 0; i < losers.size(); i++){
        TournamentNode<T>* node = new TournamentNode<T>(losers[i]);
        std::cout << "Created leaf node for loser: " << losers[i]->name << std::endl;
